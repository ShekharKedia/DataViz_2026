...
# eu_raw <- WDI(
#   country = eu_members,
#   indicator = "NY.GDP.PCAP.CD",  # GDP per capita (USD)
#   start = 1995,
#   end = 2023
# )
# 
...
##Area1 = 1 (Due Process) is the most common constitutional issue reviewed.

# ##To make it interpretable, we can label the areas
# area_labels <- c(
#   "1" = "Due Process",
...
# 
# jr |>
#   mutate(area_label = area_labels[as.character(AREA1)]) |>
#   count(area_label, sort = TRUE)
...
##Or, cases_by_year <- jr |> group_by(YEAR) %>% count()
...
##Correlogram: quickly shows strength & direction
##Scatterplot matrix: reveals distribution, non-linearity and outliers

# Which visualization would you include in a paper, and why?

##Correlogram (compact, easily interpretable)
...
##Interpretation: The model explains TIME TO DECISION (in months) for Supreme Court cases reviewing 
##Acts of Congress as a function of case outcome, historical context, government participation, and internal disagreement.

##1.Each more recent Congress that enacted the statute is associated with a 6-month reduction in time to decision or, 
##Statutes enacted more recently are reviewed more quickly.

##2.Each additional dissenting justice increases time to decision by approximately 24 months.
...
##The predicted values show that greater dissent is associated with substantially longer times to decision in Supreme Court cases reviewing Acts of Congress. 
##Each additional dissenting justice increases deliberation time by roughly two years, indicating that internal disagreement slows the Court’s decision-making process.
